---
title: "Hello"
draft: false
baseURL: "https://andrewdea.github.io/"
---
Here's some things I've done:
 
#### Software
 - [MTA info](https://github.com/andrewdea/mta-info): all NYC bus stops in [one map](https://www.google.com/maps/d/u/0/viewer?mid=1Y-euNeFcsu06Zxfdl6u6-sca3Yp-KYY&ll=40.71865725691622%2C-73.976623&z=10)
 - [sticky shell](https://github.com/andrewdea/sticky-shell): minor Emacs mode to keep track of previous prompts in your shell
 - [icloud-utilities](https://github.com/andrewdea/icloud-utilities): easily download iCloud files from within Emacs
 
I've contributed to:
 - [Emacs](https://git.savannah.gnu.org/cgit/emacs.git/commit/?id=a137f71c67e88204a32ebd747beb8fdd7db2fbe9): fixed memory-leaks and updated the [xwidget](https://emacsdocs.org/docs/elisp/Xwidgets) feature, which allows users to open a browser within their favorite editor
 - [complexipy](https://github.com/rohaquinlop/complexipy): a library to compute the cognitive complexity of Python code
 - [Codellm Devkit](https://github.com/codellm-devkit) a program-analysis framework for LLM-assisted software development

#### Organizing
 - I'm a proud member of the [Crown Heights Tenant Union](https://www.crownheightstenantunion.org/), organizing for a better future with neighbors in my community and beyond.
 - I co-founded the [Sanctuary Jam](https://bit.ly/sanctuary-jam-info), an open jam session in Crown Heights, where all community members can learn from each other, create, and build relationships.

#### Music
 - [Scores](https://musescore.com/user/37109046)
 - [Remixes and electronic pieces](https://soundcloud.com/beat-making-man/popular-tracks)
 - [Some older pieces](https://soundcloud.com/bobo_aka_me/zx-1)


